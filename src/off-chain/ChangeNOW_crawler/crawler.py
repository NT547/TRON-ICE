...
from storage import JsonlWriter, transaction_to_record, utc_now_iso

logger = logging.getLogger(__name__)


class ChangeNowCrawler:
    def __init__(
        self,
    ...
    def resume(self) -> int:
        ...
        logger.info("Resume: %s dedup keys from %s", loaded, self.output_file)
        return loaded
    ...
    def _process_transactions(self, txs: list[dict[str, Any]]) -> int:
        ...
        for tx in txs:
            if self._stop:
                break
            record = transaction_to_record(tx, observed_at=observed_at)
            if not record.get("input_coin") or not record.get("output_coin"):
                continue
    ...
    def poll_once(self) -> int:
        txs = self.client.fetch_recent_transactions(status=self.status_filter)
        self.stats["polls"] += 1
        self.stats["fetched"] += len(txs)
        appended = self._process_transactions(txs)
        self._save_state()
        logger.info(
    ...
    def run(self) -> None:
        ...
        logger.info("Starting ChangeNOW crawler. Output: %s", self.output_file)
        self.resume()
        ...
        try:
            while not self._stop:
                try:
                    ...
                    logger.error("ChangeNOW API auth/permissions error; stopping crawler.")
                    raise
                except Exception:
                    self.stats["errors"] += 1
                    logger.exception("Poll failed; retry after interval.")
                if self._stop:
                    break
