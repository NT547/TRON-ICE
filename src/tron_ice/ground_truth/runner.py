...
from src.tron_ice.ground_truth.export_labels import export_training_pairs
from src.tron_ice.ground_truth.io import (
...
from src.tron_ice.ground_truth.match import GroundTruthMatcher, MatchConfig
from src.tron_ice.ground_truth.paths import (
    default_outputs,
    offchain_history_path,
    priced_paths,
    tron_classified_paths,
)
from src.tron_ice.ground_truth.schemas import format_ground_truth_record
from src.tron_ice.ground_truth.trace import TraceConfig, TronMultiHopTracer
...
def build_ground_truth(
    ...
    *,
    ...
    """Compatibility wrapper for the CLI to run the ground-truth stage."""
    cfg = GroundTruthRunConfig(
        service=service,
        year=year,
        trace_depth=trace_depth,
        export_training=export_training,
        history_network_filter=history_network_filter,
    )
    return run_ground_truth(cfg)
...
def _filter_history_by_network(
    ...
    if mode not in {"tron-any", "tron-both"}:
        raise ValueError(
            "history_network_filter must be one of: all, tron-any, tron-both"
        )
...
def _load_onchain(
    cfg: GroundTruthRunConfig, history: list[dict[str, Any]]
...
def _load_onchain_with_window(
    ...
    if cfg.deposits_json and cfg.withdrawals_json:
        ...
        withdrawals = load_tron_classified_json(
            cfg.tron_withdrawals_json, min_ts, max_ts
        )
    else:
...
def run_ground_truth(cfg: GroundTruthRunConfig) -> dict[str, Any]:
    ...
    logger.info(
        "On-chain loaded: deposits=%s withdrawals=%s", len(deposits), len(withdrawals)
    )
    ...
    if cfg.trace_depth > 0:
        tracer = TronMultiHopTracer(
            TronGridClient(),
            TraceConfig(
                max_depth=cfg.trace_depth,
                hop_window_ms=cfg.trace_window_min * 60 * 1000,
            ),
        )
        logger.info("TRON trace depth=%s", cfg.trace_depth)

    matcher = GroundTruthMatcher(
        match_cfg, deposits=deposits, withdrawals=withdrawals, tracer=tracer
    )
